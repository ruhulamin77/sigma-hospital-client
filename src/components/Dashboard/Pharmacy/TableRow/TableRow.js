...
const TableRow = ({ medicine, index }) => {
  ...
  const onSubmit = (data) => {
    ...
    axios
      .put(
        `https://sigma-hospital-server.onrender.com/medicine/order/${data._id}`,
        {
          stock: { data },
        }
      )
  ...
  return (
    <>
      <tr>
        ...
        <td>{medicine?.type}</td>
        <td className={medicine?.stock === 0 ? 'stockOut' : ' '}>
          {medicine?.stock}
        </td>
        ...
        <td className="quantity">
          <input
            type="number"
            readOnly
            value={medicine?.salePrice * quantity}
            className="medicen-input-fild"
          />
        </td>
        <td>
