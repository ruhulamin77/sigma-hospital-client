...
const Messenger = () => {
    ...
    useEffect(() => {
        ...
        const getData = async () => {
            try {
                const res = await axios.get(`https://shrouded-headland-44423.herokuapp.com/adminUser/${admin?.adminEmail}`)
    ...
    useEffect(() => {
        axios.get("https://shrouded-headland-44423.herokuapp.com/adminUsers").then(data => setUsers(data))
    }, [loginUsers?._id])
