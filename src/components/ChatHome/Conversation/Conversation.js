    ...
    const getFriend = async () => {
      try {
        const res = await axios.get(
          `https://sigma-hospital-server.onrender.com/getUsers/${firend}`
        );
  ...
  const handleDeleteConversation = (conversationId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(
          `https://sigma-hospital-server.onrender.com/conversationDelete/${conversationId}`,
          {
            method: 'DELETE',
    ...
    <div className="conversation">
      <div>
        <img
          ...
          alt=""
        />
        <span className="conversation-name">{friendId?.adminName}</span>
      </div>
      <span
