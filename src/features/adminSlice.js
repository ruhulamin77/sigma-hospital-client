...
export const adminRegister = createAsyncThunk('adminRegister', async (body) => {
  const result = await adminData(
    'https://sigma-hospital-server.onrender.com/adminRegistar',
    body
  );
  return result;
});
export const adminLogin = createAsyncThunk('adminLogin', async (body) => {
  const result = await adminData(
    'https://sigma-hospital-server.onrender.com/adminLogin',
    body
  );
  return result;
});
const adminSlice = createSlice({
  ...
  extraReducers: {
    ...
    [adminLogin.fulfilled]: (
      state,
      { payload: { error, token, displayName, photoURL, role, adminEmail } }
    ) => {
      state.loading = false;
      if (error) {
        state.error = error;
